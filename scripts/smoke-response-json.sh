...
ROOT=${ROOT:-.tmp-smoke/live-response-json}
GATEWAY_PORT=${GATEWAY_PORT:-18099}
UPSTREAM_PORT=${UPSTREAM_PORT:-19119}
OPA_PORT=${OPA_PORT:-18183}
...
root.joinpath("mode.txt").write_text("review\n", encoding="utf-8")
PY
...
curl -fsS -D "$ROOT/response-headers.txt" \
...
root = Path(os.environ["ROOT"])
raw_body = root.joinpath("response-body.json").read_text(encoding="utf-8")
...
assert payload["error"] == "response redacted by gateway", payload
assert "普通响应内容" not in raw_body, raw_body
...
assert "content-length: 40" in headers, headers
...
assert response_record["decision_reason"] == "response requires approval"
...
print("smoke_response_json_ok")
print("buffered json response redacted before client")
print("content-length rebuilt for redacted body")
